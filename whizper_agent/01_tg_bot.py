...
def _dexscreener_token_url_from_pair(pair: dict) -> str | None:
    chain = (pair.get("chainId") or "").lower()
    token_addr = ((pair.get("baseToken") or {}).get("address") or "").strip()
    if chain and token_addr:
        return f"https://dexscreener.com/{chain}/{token_addr}"
    return None
def _format_usd(v) -> str:
    try:
        return f"${float(v):,.6f}" if float(v) < 1 else f"${float(v):,.2f}"
    except Exception:
        return "N/A"
def _format_int_usd(v) -> str:
    try:
        return f"${int(float(v)):,}"
    except Exception:
        return "N/A"
async def _dexscreener_best_snapshot(sess: aiohttp.ClientSession, address: str) -> dict | None:
    chain = _guess_chain(address)
    if chain:
        p = await _dexscreener_pairs_lookup(sess, chain, address)
        if p:
            return p
    return await _dexscreener_search_lookup(sess, address)
    ...
    for base in BINANCE_BASES:
        try:
            ...
            cols = [
                ...
                "num neuroimaging","taker_buy_base","taker_buy_quote","ignore"
            ]
...
def _contract_retry_keyboard(address_hint: str | None = None) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [
            ...
            InlineKeyboardButton("🦎 GeckoT", url=gt),
        ]
    ])
WELCOME = (
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        ds_pair = await _dexscreener_best_snapshot(sess, address)
        ds_url = _dexscreener_token_url_from_pair(ds_pair) if ds_pair else None
        name = ((ds_pair or {}).get("baseToken") or {}).get("name") or base_sym
        symbol = ((ds_pair or {}).get("baseToken") or {}).get("symbol") or base_sym
        price = _format_usd((ds_pair or {}).get("priceUsd"))
        fdv = (ds_pair or {}).get("fdv") or (ds_pair or {}).get("marketCap")
        fdv_fmt = _format_int_usd(fdv)
        chain = (ds_pair or {}).get("chainId") or net
        snapshot_lines = [
            f"{name} (${symbol}) on {str(chain).title()}",
            f"Price: {price}",
            f"Market Cap/FDV: {fdv_fmt}",
        ]
        snapshot = "\n".join(snapshot_lines)
        context.user_data.pop("awaiting_contract", None)
        gt_url = f"https://www.geckoterminal.com/{net}/pools/{pool}"
        dex_button_url = ds_url or f"https://dexscreener.com/search?q={urllib.parse.quote_plus(address)}"
        kb = _contract_action_keyboard(dex_button_url, gt_url)
        header = f"📈 Whispering {pair.upper()} on {interval}:\n\n"
        if ds_url:
            await update.message.reply_text(snapshot + "\n" + ds_url, reply_markup=kb, disable_web_page_preview=False)
        else:
            await update.message.reply_text(snapshot, reply_markup=kb)
